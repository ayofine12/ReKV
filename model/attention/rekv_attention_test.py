...
def __main__():
    ...
    print(f"\n{'='*60}")
    print(f"Pipeline Summary (ENABLE_OVERLAP={ENABLE_OVERLAP}):")
    print(f"{'='*60}")
    ...
    sequential_time = (append_gpu0_time_ms + transfer_gpu0_to_gpu1_time_ms + 
                      append_gpu1_time_ms + transfer_gpu1_to_gpu2_time_ms + 
                      append_gpu2_time_ms)
    time_saved = sequential_time - total_pipeline_time
    
    print(f"Total pipeline time:             {total_pipeline_time:.3f} ms")
    ...
    print(f"")
    print(f"Sequential time would be:        {sequential_time:.3f} ms")
    print(f"Time saved by overlap:           {time_saved:.3f} ms ({time_saved/sequential_time*100:.1f}%)")
    print(f"{'='*60}\n")
